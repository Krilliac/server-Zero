...
namespace
{
    // Tolerances are deliberately generous in Slice 1 to keep false positives
    // near zero; tuning happens from real logs before enforcement is enabled.
...
namespace PhysicsValidator
{
                                    ...
                                    MovementInfo const& mi, const char** reason)
    {
        ...
        // Only the grounded case has a cheap, reliable terrain expectation in
        // Slice 1. Falling/flying/swimming/transport legitimately deviate from
        // the heightmap, so we do not judge them here (deferred to later slices).
