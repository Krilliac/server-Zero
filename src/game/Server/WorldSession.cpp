...
// Anti-Cheat time-sync: fold a fresh client-reported latency sample into the
// rolling window and EWMA. Fed from WorldSocket::HandlePing (CMSG_PING).
void WorldSession::UpdateLatencyStats(uint32 sampleMS)
{
    ...
    // Desync detection: a sample that deviates wildly from the established
    // baseline (latency spoofing / time manipulation). Once the window is full,
    // compare against the PREVIOUS smoothed value before folding the new sample.
    // Flagged here (network thread); recorded later on the safe thread.
    if (m_latCount >= LATENCY_WINDOW && m_latEWMA > 0)
    {
    ...
    // EWMA with alpha (percent) from config: ewma = a*sample + (1-a)*ewma.
