...
void WorldSession::HandleMovementOpcodes(WorldPacket& recvPacket)
{
    ...
    if (!_player || !_player->IsAlive() || _player->IsBeingTeleported()) {
        return;
    }

    // Time synchronization validation
    if (!sTimeSyncMgr->ValidateMovementTime(_player, movementInfo.GetTime()))
    {
        int32 drift = sTimeSyncMgr->GetTimeOffset(_player->GetGUID());
        sAnticheatMgr->RecordViolation(_player, CHEAT_TIME_DESYNC,
            fmt::format("Time desync: Drift={}ms | PacketTime={} | ServerTime={}",
                drift,
                movementInfo.GetTime(),
                WorldTimer::getMSTime()));
        
        // Apply corrective action based on config
        switch (sConfig.GetIntDefault("TimeSync.CorrectiveAction", 1)) {
            case 1: // Position correction
                _player->TeleportTo(_player->GetPositionX(), 
	...
	// Process movement based on opcode type
	switch (recvPacket.GetOpcode())
	{
		case MSG_MOVE_HEARTBEAT:
			HandleMoveHeartbeat(movementInfo);
			break;
		case MSG_MOVE_JUMP:
			HandleMoveJump(movementInfo);
			break;
		case MSG_MOVE_START_FORWARD:
			HandleMoveStartForward(movementInfo);
			break;
		case MSG_MOVE_START_BACKWARD:
			HandleMoveStartBackward(movementInfo);
			break;
		case MSG_MOVE_STOP:
			HandleMoveStop(movementInfo);
			break;
		case MSG_MOVE_START_STRAFE_LEFT:
			HandleMoveStartStrafeLeft(movementInfo);
			break;
		case MSG_MOVE_START_STRAFE_RIGHT:
			HandleMoveStartStrafeRight(movementInfo);
			break;
		case MSG_MOVE_STOP_STRAFE:
			HandleMoveStopStrafe(movementInfo);
			break;
		case MSG_MOVE_START_TURN_LEFT:
			HandleMoveStartTurnLeft(movementInfo);
			break;
		case MSG_MOVE_START_TURN_RIGHT:
			HandleMoveStartTurnRight(movementInfo);
			break;
		case MSG_MOVE_STOP_TURN:
			HandleMoveStopTurn(movementInfo);
			break;
		case MSG_MOVE_START_PITCH_UP:
			HandleMoveStartPitchUp(movementInfo);
			break;
		case MSG_MOVE_START_PITCH_DOWN:
			HandleMoveStartPitchDown(movementInfo);
			break;
		case MSG_MOVE_STOP_PITCH:
			HandleMoveStopPitch(movementInfo);
			break;
		case MSG_MOVE_SET_RUN_MODE:
			HandleMoveSetRunMode(movementInfo);
			break;
		case MSG_MOVE_SET_WALK_MODE:
			HandleMoveSetWalkMode(movementInfo);
			break;
		case MSG_MOVE_FALL_LAND:
			HandleMoveFallLand(movementInfo);
			break;
		case MSG_MOVE_START_SWIM:
			HandleMoveStartSwim(movementInfo);
			break;
		case MSG_MOVE_STOP_SWIM:
			HandleMoveStopSwim(movementInfo);
			break;
		case MSG_MOVE_SET_FACING:
			HandleMoveSetFacing(movementInfo);
			break;
		default:
    ...
    _player->SetPosition(movementInfo.GetPos());
...
void WorldSession::HandleMoveHeartbeat(MovementInfo const& moveInfo)
{
    _player->m_movementInfo = moveInfo;
    _player->UpdatePosition(moveInfo.GetPos());
}

void WorldSession::HandleMoveJump(MovementInfo const& moveInfo)
{
    _player->m_movementInfo = moveInfo;
    _player->SetFallInformation(0, moveInfo.GetPos().z);
}

void WorldSession::HandleMoveStartForward(MovementInfo const& moveInfo)
{
    _player->m_movementInfo = moveInfo;
    _player->SetMovementFlags(MOVEFLAG_FORWARD);
}

void WorldSession::HandleMoveStartBackward(MovementInfo const& moveInfo)
{
    _player->m_movementInfo = moveInfo;
    _player->SetMovementFlags(MOVEFLAG_BACKWARD);
}

void WorldSession::HandleMoveStop(MovementInfo const& moveInfo)
{
    _player->m_movementInfo = moveInfo;
    _player->RemoveMovementFlags(MOVEFLAG_MASK_MOVING);
}

void WorldSession::HandleMoveStartStrafeLeft(MovementInfo const& moveInfo)
{
    _player->m_movementInfo = moveInfo;
    _player->SetMovementFlags(MOVEFLAG_STRAFE_LEFT);
}

void WorldSession::HandleMoveStartStrafeRight(MovementInfo const& moveInfo)
{
    _player->m_movementInfo = moveInfo;
    _player->SetMovementFlags(MOVEFLAG_STRAFE_RIGHT);
}

void WorldSession::HandleMoveStopStrafe(MovementInfo const& moveInfo)
{
    _player->m_movementInfo = moveInfo;
    _player->RemoveMovementFlags(MOVEFLAG_STRAFE_LEFT | MOVEFLAG_STRAFE_RIGHT);
}

void WorldSession::HandleMoveStartTurnLeft(MovementInfo const& moveInfo)
{
    _player->m_movementInfo = moveInfo;
    _player->SetMovementFlags(MOVEFLAG_TURN_LEFT);
}

void WorldSession::HandleMoveStartTurnRight(MovementInfo const& moveInfo)
{
    _player->m_movementInfo = moveInfo;
    _player->SetMovementFlags(MOVEFLAG_TURN_RIGHT);
}

void WorldSession::HandleMoveStopTurn(MovementInfo const& moveInfo)
{
    _player->m_movementInfo = moveInfo;
    _player->RemoveMovementFlags(MOVEFLAG_TURN_LEFT | MOVEFLAG_TURN_RIGHT);
}

void WorldSession::HandleMoveStartPitchUp(MovementInfo const& moveInfo)
{
    _player->m_movementInfo = moveInfo;
    _player->SetMovementFlags(MOVEFLAG_PITCH_UP);
}

void WorldSession::HandleMoveStartPitchDown(MovementInfo const& moveInfo)
{
    _player->m_movementInfo = moveInfo;
    _player->SetMovementFlags(MOVEFLAG_PITCH_DOWN);
}

void WorldSession::HandleMoveStopPitch(MovementInfo const& moveInfo)
{
    _player->m_movementInfo = moveInfo;
    _player->RemoveMovementFlags(MOVEFLAG_PITCH_UP | MOVEFLAG_PITCH_DOWN);
}

void WorldSession::HandleMoveSetRunMode(MovementInfo const& moveInfo)
{
    _player->m_movementInfo = moveInfo;
    _player->RemoveUnitMovementFlag(MOVEFLAG_WALK_MODE);
}

void WorldSession::HandleMoveSetWalkMode(MovementInfo const& moveInfo)
{
    _player->m_movementInfo = moveInfo;
    _player->AddUnitMovementFlag(MOVEFLAG_WALK_MODE);
}

void WorldSession::HandleMoveFallLand(MovementInfo const& moveInfo)
{
    _player->m_movementInfo = moveInfo;
    _player->SetFallInformation(0, moveInfo.GetPos().z);
    _player->UpdateFallDamageIfNeed();
}

void WorldSession::HandleMoveStartSwim(MovementInfo const& moveInfo)
{
    _player->m_movementInfo = moveInfo;
    _player->SetWaterWalking(false);
    _player->SetSwimState(true);
}

void WorldSession::HandleMoveStopSwim(MovementInfo const& moveInfo)
{
    _player->m_movementInfo = moveInfo;
    _player->SetSwimState(false);
}

void WorldSession::HandleMoveSetFacing(MovementInfo const& moveInfo)
{
    _player->m_movementInfo = moveInfo;
    _player->SetFacingTo(moveInfo.GetOrientation());
}
